...
class CrawlerPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if item.get("property_type") not in [
            "apartment",
            "house",
            "room",
            "property_for_sale",
            ...
            raise DropItem(
                f"Item dropped with property_type {item.get('property_type')}"
            )

        item["external_images_count"] = len(item.get("images", [])) + len(
            item.get("floor_plan_images", [])
        )
        ...
        for r in required:
            if r not in item:
                raise ValueError(
                    f"Missing required field {r} from {item.get('external_link')}"
                )
...
class CustomImagePipeLine(ImagesPipeline):
    ...
    def file_path(self, request, response=None, info=None):
        path = "{}/{}".format(self.spider_name, request.meta["file_name"])
        return path
...
class MultiCSVItemPipeline(object):
    ...
    def spider_opened(self, spider):
        self.files = (
            dict()
        )  # dict([ (name, open(CSVDir + name + '.csv','w+b')) for name in self.SaveTypes ])
        self.exporters = (
            dict()
        )  # dict([ (name,CsvItemExporter(self.files[name])) for name in self.SaveTypes])
    ...
    def process_item(self, item, spider):
        ...
        if file_key not in self.files:
            self.files[file_key] = open("output/group/" + file_key + ".json", "w+b")
